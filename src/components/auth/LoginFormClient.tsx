...
export default function LoginFormClient() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // Determine target based on onboarding status
      const targetUrl = !result.user.onboarded
        ? '/onboarding'
  ...
  return (
    ...
    >
      ...
      <div className="text-center text-xs text-gray-500 font-semibold">
        <Link href="/forgot-password" className="text-gray-950 font-bold hover:underline">
          Forgot Password?
        </Link>
      </div>
    </form>
