...
export default function GlassWindow({ children }: GlassWindowProps) {
  return (
    <div className="min-h-screen w-full bg-[#B8B5B0] relative overflow-hidden flex flex-col">
      ...
      {/* Outer container to enforce 28px (m-7) margins */}
      <div className="flex-1 flex flex-col m-7 min-h-[calc(100vh-56px)] bg-white/20 backdrop-blur-3xl border border-white/50 rounded-[24px] shadow-2xl overflow-hidden relative">
        ...
        {/* Interior surface */}
        <div className="flex-1 bg-[#F5F5F3] m-1 rounded-[20px] overflow-auto relative flex flex-col">
          {children}
        </div>
      </div>
    </div>
